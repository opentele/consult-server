...
create table consultation_session_record_file (id  serial not null, inactive boolean default false not null, uuid uuid default uuid_generate_v4() not null, created_date timestamp default (now()):: timestamp without time zone not null, last_modified_date timestamp default (now()):: timestamp without time zone not null, file_name varchar(255) not null, name varchar(255), created_by_id int4 not null, last_modified_by_id int4 not null, organisation_id integer not null, consultation_session_record_id integer not null, primary key (id));
