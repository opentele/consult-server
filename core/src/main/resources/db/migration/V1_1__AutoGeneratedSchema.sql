create table appointment_token (id  serial not null, created_date timestamp default (now()):: timestamp without time zone not null, inactive boolean, last_modified_date timestamp default (now()):: timestamp without time zone not null, uuid uuid default uuid_generate_v4() not null, queue_number int4 not null, organisation_id int4, client_id int4, consultation_room_id int4, primary key (id));
create table client (id  serial not null, created_date timestamp default (now()):: timestamp without time zone not null, inactive boolean, last_modified_date timestamp default (now()):: timestamp without time zone not null, uuid uuid default uuid_generate_v4() not null, name varchar(255), organisation_id int4, primary key (id));
create table consultant_room_schedule_user (id  serial not null, created_date timestamp default (now()):: timestamp without time zone not null, inactive boolean, last_modified_date timestamp default (now()):: timestamp without time zone not null, uuid uuid default uuid_generate_v4() not null, organisation_id int4, consultation_room_schedule_id int4, user_id int4, primary key (id));
create table consultant_room_user (id  serial not null, created_date timestamp default (now()):: timestamp without time zone not null, inactive boolean, last_modified_date timestamp default (now()):: timestamp without time zone not null, uuid uuid default uuid_generate_v4() not null, organisation_id int4, consultation_room_id int4, user_id int4, primary key (id));
create table consultation_room (id  serial not null, created_date timestamp default (now()):: timestamp without time zone not null, inactive boolean, last_modified_date timestamp default (now()):: timestamp without time zone not null, uuid uuid default uuid_generate_v4() not null, ended_at time, scheduled_end_at timestamp not null, scheduled_start_at timestamp not null, started_at time, title varchar(255) not null, total_slots int4, organisation_id int4, consultation_room_schedule_id int4, primary key (id));
create table consultation_room_schedule (id  serial not null, created_date timestamp default (now()):: timestamp without time zone not null, inactive boolean, last_modified_date timestamp default (now()):: timestamp without time zone not null, uuid uuid default uuid_generate_v4() not null, end_time time not null, recurrence_rule varchar(255) not null, start_date date not null, start_time time not null, title varchar(255) not null, total_slots int4 check (total_slots > 0) not null, organisation_id int4, primary key (id));
create table organisation (id  serial not null, created_date timestamp default (now()):: timestamp without time zone not null, inactive boolean, last_modified_date timestamp default (now()):: timestamp without time zone not null, uuid uuid default uuid_generate_v4() not null, name varchar(255) not null, primary key (id));
...
create table privilege (id  serial not null, created_date timestamp default (now()):: timestamp without time zone not null, inactive boolean, last_modified_date timestamp default (now()):: timestamp without time zone not null, uuid uuid default uuid_generate_v4() not null, name varchar(255) not null, primary key (id));
create table role (id  serial not null, created_date timestamp default (now()):: timestamp without time zone not null, inactive boolean, last_modified_date timestamp default (now()):: timestamp without time zone not null, uuid uuid default uuid_generate_v4() not null, name varchar(255) not null, primary key (id));
create table role_privilege (role_id int4 not null, privilege_id int4 not null, primary key (role_id, privilege_id));
create table users (id  serial not null, created_date timestamp default (now()):: timestamp without time zone not null, inactive boolean, last_modified_date timestamp default (now()):: timestamp without time zone not null, uuid uuid default uuid_generate_v4() not null, email varchar(255), mobile varchar(255), name varchar(255) not null, password varchar(255) not null, user_type varchar(255) not null, organisation_id int4, primary key (id));
...
alter table if exists appointment_token add constraint FK551nj0c2jibh8l463vf0h901j foreign key (consultation_room_id) references consultation_room_schedule;
